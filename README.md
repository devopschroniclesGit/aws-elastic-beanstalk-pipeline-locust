...
## Troubleshooting: Legacy Locust Syntax

Issue:
ImportError: The HttpLocust class has been renamed to HttpUser

Cause:
AWS sample repository uses older Locust syntax incompatible with Locust 1.x+

Fix:
Updated `HttpLocust` to `HttpUser`
Replaced `task_set` with inline `@task`
Added `wait_time` using `between()`

Outcome:
Locust UI launched successfully on port 8089
